...
class Renren(object):
    ...
    def login(self, email='13613112728', password='075100', origURL='http://www.renren.com/PLogin.do'):
    #def login(self, email='', password='', origURL=''):
        ...
        while not self.is_login:
            ...
            if ruid:
                self.is_login = True
                print "用户  %s %s" % (ruid.group(1), config.FAILCODE['-1'])
                return True
            else:
